      ...
      {visible.map(([label, value]) => {
        ...
        return isLong ? (
          ...
          <div key={label} className="flex min-w-0 items-baseline justify-between gap-3 border-b border-slate-100 py-2">
            <dt className="min-w-0 text-[13px] font-semibold leading-snug text-slate-500">{label}</dt>
            <dd className="shrink-0 whitespace-nowrap text-right text-sm font-bold text-slate-900">{text}</dd>
          </div>
        );
      })}
    </dl>
