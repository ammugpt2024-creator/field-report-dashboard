...
const TABS = [
  { id: "all", label: "All" },
  { id: GRADATION_STATUS.DRAFT, label: "Draft" },
  { id: GRADATION_STATUS.SUBMITTED, label: "Submitted" },
  { id: GRADATION_STATUS.APPROVED, label: "Approved" }
];

function statusPill(status) {
  if (status === GRADATION_STATUS.APPROVED) return "bg-emerald-100 text-emerald-800";
  if (status === GRADATION_STATUS.SUBMITTED) return "bg-blue-100 text-blue-800";
  return "bg-amber-100 text-amber-900";
}

function GradationList() {
  const navigate = useNavigate();
  const [reports, setReports] = useState(() => getGradationReports());
  const [activeTab, setActiveTab] = useState("all");
  const [search, setSearch] = useState("");

  const counts = useMemo(() => ({
    all: reports.length,
    [GRADATION_STATUS.DRAFT]: reports.filter((r) => r.status === GRADATION_STATUS.DRAFT).length,
    [GRADATION_STATUS.SUBMITTED]: reports.filter((r) => r.status === GRADATION_STATUS.SUBMITTED).length,
    [GRADATION_STATUS.APPROVED]: reports.filter((r) => r.status === GRADATION_STATUS.APPROVED).length
  }), [reports]);
  ...
  function removeReport(id) {
    if (!window.confirm("Delete this gradation report?")) return;
    deleteGradationReport(id);
    setReports(getGradationReports());
  }
          ...
          <div className="border-b-4 border-accent-500 bg-gradient-to-br from-navy-800 via-navy-900 to-navy-950 px-5 py-6 sm:px-7">
            ...
            <div className="mt-1 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <h1 className="mt-1 text-2xl font-bold tracking-tight text-white sm:text-3xl">Gradation Reports</h1>
              </div>
              <button type="button" onClick={() => navigate("/technician/lab/gradation/new")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-accent-500 px-5 text-sm font-bold text-white shadow-lg shadow-accent-950/30 transition hover:bg-accent-600">
          ...
          <div className="mt-4 space-y-2">
            {filtered.map((report) => (
              <article
                ...
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/technician/lab/gradation/${report.id}`)}
                onKeyDown={(event) => { if (event.key === "Enter") navigate(`/technician/lab/gradation/${report.id}`); }}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-[13px] font-medium text-slate-500">Boring {report.boringNumber || "-"}</p>
                </div>
                <p><span className={`inline-flex rounded-full px-2.5 py-0.5 text-[13px] font-semibold ${statusPill(report.status)}`}>{formatGradationStatus(report.status)}</span></p>
                ...
                <div className="flex flex-nowrap items-center gap-2 lg:justify-end" onClick={(event) => event.stopPropagation()}>
                  <button type="button" onClick={() => openGradationPdf(report)} className="inline-flex h-9 items-center justify-center rounded-lg border border-slate-300 bg-white px-3 text-[13px] font-semibold text-slate-700 transition hover:bg-slate-50">PDF</button>
            ...
            {!filtered.length && (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
                ...
                <p className="mt-3 text-sm font-semibold text-slate-600">No gradation reports yet.</p>
                <button type="button" onClick={() => navigate("/technician/lab/gradation/new")} className="mt-3 inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white hover:bg-slate-800"><Plus className="h-4 w-4" /> New Report</button>
              </div>
            )}
          </div>
        </section>
