...
import {
  ASPHALT_BSG_STATUS,
  deleteAsphaltGravityReport,
  formatAsphaltGravityStatus,
  getAsphaltGravityReports
} from "../services/asphaltGravityService";
import { openAsphaltGravityPdf } from "../services/asphaltGravityPdfService";

const TABS = [
  { id: "all", label: "All" },
  { id: ASPHALT_BSG_STATUS.DRAFT, label: "Draft" },
  { id: ASPHALT_BSG_STATUS.SUBMITTED, label: "Submitted" },
  { id: ASPHALT_BSG_STATUS.APPROVED, label: "Approved" }
];

function statusPill(status) {
  if (status === ASPHALT_BSG_STATUS.APPROVED) return "bg-emerald-100 text-emerald-800";
  if (status === ASPHALT_BSG_STATUS.SUBMITTED) return "bg-blue-100 text-blue-800";
  return "bg-amber-100 text-amber-900";
}

function AsphaltGravityList() {
  const navigate = useNavigate();
  const [reports, setReports] = useState(() => getAsphaltGravityReports());
  const [activeTab, setActiveTab] = useState("all");
  const [search, setSearch] = useState("");

  const counts = useMemo(() => ({
    all: reports.length,
    [ASPHALT_BSG_STATUS.DRAFT]: reports.filter((r) => r.status === ASPHALT_BSG_STATUS.DRAFT).length,
    [ASPHALT_BSG_STATUS.SUBMITTED]: reports.filter((r) => r.status === ASPHALT_BSG_STATUS.SUBMITTED).length,
    [ASPHALT_BSG_STATUS.APPROVED]: reports.filter((r) => r.status === ASPHALT_BSG_STATUS.APPROVED).length
  }), [reports]);
  ...
  function removeReport(id) {
    if (!window.confirm("Delete this bulk specific gravity report?")) return;
    deleteAsphaltGravityReport(id);
    setReports(getAsphaltGravityReports());
  }
          ...
          <div className="mt-4 space-y-2">
            {filtered.map((report) => (
              <article
                ...
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/technician/lab/asphalt-bsg/${report.id}`)}
                onKeyDown={(event) => { if (event.key === "Enter") navigate(`/technician/lab/asphalt-bsg/${report.id}`); }}
              ...
              >
                ...
                <p><span className={`inline-flex rounded-full px-2.5 py-0.5 text-[13px] font-semibold ${statusPill(report.status)}`}>{formatAsphaltGravityStatus(report.status)}</span></p>
                ...
                <div className="flex flex-nowrap items-center gap-2 lg:justify-end" onClick={(event) => event.stopPropagation()}>
                  <button type="button" onClick={() => openAsphaltGravityPdf(report)} className="inline-flex h-9 items-center justify-center rounded-lg border border-slate-300 bg-white px-3 text-[13px] font-semibold text-slate-700 transition hover:bg-slate-50">PDF</button>
            ...
            {!filtered.length && (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
                ...
                <p className="mt-3 text-sm font-semibold text-slate-600">No bulk specific gravity reports yet.</p>
                <button type="button" onClick={() => navigate("/technician/lab/asphalt-bsg/new")} className="mt-3 inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white hover:bg-slate-800"><Plus className="h-4 w-4" /> New Report</button>
              </div>
            )}
          </div>
        </section>
...
export default AsphaltGravityList;
